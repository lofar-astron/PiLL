...
def plugin_main(args, **kwargs):
	...
	for groupID,freq_group in enumerate(frequency_groups):
		if freq_group[0] <= ref_frequency <= freq_group[1]:
			if tomap <= msID:
				map_out.data.append(DataProduct(data[msID].host, data[msID].file, (data[msID].skip or groups[groupID].skip)))
				map_out_parmdbs.data.append(DataProduct(groups[groupID].host, groups[groupID].file, (groups[groupID].skip or data[msID].skip)))
				tomap += 1
				pass
			break
			pass
    ...
    if tomap == 0:
        raise ValueError('PipelineStep_FindCorrespondingMSX: There is no parmdb matching a target measurement set. Check your frequency range!')


    fileid = os.path.join(mapfile_dir, filename + '_parmdbs')
    map_out_parmdbs.save(fileid)
